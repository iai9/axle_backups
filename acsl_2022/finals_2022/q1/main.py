'''
alright so i have to make a function that determines the y,g,- of a word compared to the target word and then I 
need to make logic to rank word guess word value. 
...
def develop_colors(target, guess): # target is a list of 5 letters and so is guess
    ...
    for idx in range(len(guess)): # i shouldve put this above green but whatever it works and im timed
        if t_guess[idx] == 0:
            continue
        elif (t_guess[idx] in t_target) and (t_guess[idx] not in used):
...
print(develop_colors(list("trees"),list("start")))
print(develop_colors(list("trees"),list("eater")))
print(develop_colors(list("ports"),list("sorts")))
print(develop_colors(list("ports"),list("parts")))

# word = "ports"
# guesses = "climb spots sport parts stops traps sorts porch props shots prank"
# guesses = guesses.split(' ')
# guesses = sorted(guesses)
...
#     g = []
#     t = list(guess)
#     cinfo = develop_colors(word, t)
#     g.append(cinfo)
#     g.append(t)
#     l.append(g)

# for goofy in l:
#     a = develop_info(goofy)
#     goofy.append(a)
